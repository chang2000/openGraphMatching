...
class OrderEmbedder(nn.Module):
    def __init__(self, input_dim, hidden_dim, args):
        ...
        # output the points of query and target in embedding space
        self.emb_model = SkipLastGNN(input_dim, hidden_dim, hidden_dim, args)
        self.margin = args.margin   # 0.1
        self.use_intersection = False

        # according to E, output the probability of being subgraph
        self.clf_model = nn.Sequential(nn.Linear(1, 2), nn.LogSoftmax(dim=-1))  # classification
    ...
    def criterion(self, pred, intersect_embs, labels):
        ...
        margin = self.margin    # alpha in paper, let distance be greater than alpha
...
class SkipLastGNN(nn.Module):
    def __init__(self, input_dim, hidden_dim, output_dim, args):
        ...
        if args.skip == 'learnable':    # True, skip some features.
    ...
    def forward(self, data):
        ...
        emb = pyg_nn.global_add_pool(emb, batch)    # sum of features of values
